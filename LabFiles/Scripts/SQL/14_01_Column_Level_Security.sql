...
select  Top 100 * from dbo.WWICampaignAnalytics
...
REVOKE SELECT ON dbo.WWICampaignAnalytics FROM DataAnalystMiami;
GRANT SELECT ON dbo.WWICampaignAnalytics([Analyst], [CampaignName], [Region], [State], [City], [RevenueTarget]) TO DataAnalystMiami;
...
EXECUTE AS USER ='DataAnalystMiami';
select [Analyst],[CampaignName], [Region], [State], [City], [RevenueTarget] from dbo.WWICampaignAnalytics;
...
Revert;
GRANT SELECT ON dbo.WWICampaignAnalytics TO CEO;  --Full access to all columns.

-- Step:6 Let us check if our CEO user can see all the information that is present. Assign Current User As 'CEO' and the execute the query
EXECUTE AS USER ='CEO'
select * from dbo.WWICampaignAnalytics
Revert;